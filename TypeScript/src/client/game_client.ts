...
export class GameClient {
	...
	public constructor(url: string, token: string) {
		...
		this.connection.on("Authenticated", (...args: unknown[]) =>
	...
	public async connect(): Promise<boolean> {
		try {
			...
			console.log(
				"[INFO] Network connection established, waiting for token validation...",
			);
	...
	public backendListening(onUpdate: UpdateHandler): void {
		...
		this.connection.on("ReceiveVisibleMap", (...args: unknown[]) =>
			this.onReceiveVisibleMap(...args),
		);
		this.connection.on("ReceivePlayerInfo", (...args: unknown[]) =>
			this.onReceivePlayerInfo(...args),
		);
	}
	...
	private onTick(...args: unknown[]): void {
		try {
			...
			if (this.isServerData(data)) {
				this.currentState.CurrentTick = Number(
					data.tick ?? this.currentState.CurrentTick,
				);
			}
