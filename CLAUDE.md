...
- **Movement is grid-based, cardinal-only — no diagonals, and one tile per `MoveAction`.** The bot can only move up, down, left, or right, one step at a time: a single `MoveAction` must change either `X` or `Y` (never both), and by exactly 1 (add or subtract 1, never more). A target that differs on both axes, or by more than 1 tile, will not produce the expected movement — this was the cause of the bot appearing stuck/not moving. Never compute a distant target position directly; use the `moveTowards(to, from, state)` / `stepInDirection(direction, from, state)` helpers in `src/helpers/queries.ts` to get the single-tile step.
- **Obstacles must be checked before moving — the server won't error, it'll just silently not move the bot.** Use `isBlocked(state, position)` from `src/helpers/queries.ts` to check a tile (structure, entity, or blocking terrain category) before stepping into it; `moveTowards`/`stepInDirection` already do this, log `OBSTACLE ON THE <direction> OF THE BOT`, and pick a detour when the direct step is blocked.
- Shared, non-strategy-specific logic (distance/blocking/movement helpers, nearest-X lookups, inventory/zone checks) lives in `src/helpers/queries.ts` — put new reusable helpers there rather than in a strategy file.
